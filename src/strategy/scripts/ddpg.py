    ...
    def step(self, obs, reuse):
        with tf.variable_scope(self.name, reuse=reuse):
            ...
            h3 = tf.layers.dense(h2, 256, tf.nn.leaky_relu,
                                 kernel_initializer=tf.orthogonal_initializer(gain=np.sqrt(2)))
            h3 = tf.layers.dense(h3, 1024, tf.nn.leaky_relu,
                                 kernel_initializer=tf.orthogonal_initializer(gain=np.sqrt(2)))
...
class DDPG(object):
    def __init__(self, act_dim, obs_dim, lr_actor, lr_q_value, gamma,
        ...
        self.writer = tf.summary.FileWriter('./../DDPG_logs/'+NAME+'/'+self.name+'/'+NAME+'/'+self.name+'/', self.sess.graph)
    ...
    def choose_action(self, obs):
        ...
        obs = np.array(obs)
        action = self.sess.run(self.action, feed_dict={self.OBS0: obs.reshape(1, -1)})
