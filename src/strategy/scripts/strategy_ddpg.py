...
class Strategy(object):
    ...
    def ros_init(self):
        if self.team == 'A':
            self.agent = DDPG(act_dim=2, obs_dim=12,
                lr_actor=l_rate*(1e-3), lr_q_value=l_rate*(1e-3), gamma=0.99, tau=0.995, action_noise_std=1 )
            ...
            if self.game_is_done() and self.real_resart:
                ...
                if i > 1:
                    ...
                    if len(self.s) == 12 and len(s_) == 12:
                        ...
                        self.agent.memory.store_transition(self.s , self.a, self.r, s_, self.done)
                    # print('d',self.done)
                    self.ep_rwd = self.r
