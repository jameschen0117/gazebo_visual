...
class SAC(object):
    def __init__(self, act_dim, obs_dim, lr_actor, lr_value, gamma, tau, alpha=0.2,):
        ...
        self.IN_RATE = tf.placeholder(tf.float32, [], name=self.name+'IN_RATE')
        ...
        tf.summary.scalar(self.name+'IN_RATE', self.IN_RATE)
    ...
    def learn(self, indx, reward, ep_rwd, in_rate):
        ...
        feed_dict = {self.OBS0: obs0, self.ACT: act,self.OBS1: obs1, self.RWD: rwd,
                                                               self.DONE: np.float32(done), self.ST_RWD: reward, self.EP_RWD: ep_rwd, self.IN_RATE: in_rate}
